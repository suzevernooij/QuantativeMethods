...
for m in M:
    for p in P:
        for o in O:
            if m == 0:
                W1[m, p, o] = 0  # Set W1 to 0 when m is 0
            else:
...
for p in P:
    for m in M:
        con3[p, m] = model.addConstr(W1[m, p, 2] == X1[m, p, 2])
...
for p in P:
    for m in M:
        if m == 0:
            con4[m, p] = model.addConstr(W2[m, p, 3] == X1[m, p, 3])
        else:
            con4[m, p] = model.addConstr(W2[m, p, 3] == W2[m - 1, p, 3] + X1[m, p, 2])
...
for p in P:
    for m in M:
        for o in O:
            if m == 0:
                con6[p, m, o] = model.addConstr(W1[m, p, 2] == X1[m, p, 2])
                con6[p, m, o] = model.addConstr(W2[m, p, 3] == X1[m, p, 3])
            else:
                con6[p, m, o] = model.addConstr(W1[m, p, 2] == X1[m, p, 2])
                con6[p, m, o] = model.addConstr(W2[m, p, 3] == W1[m - 1, p, 2] + X1[m, p, 3])
