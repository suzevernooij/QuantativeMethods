...
for p in P:
    for m in M:
        if m > 0:
            con3[p, m] = model.addConstr(W2[m, p, 3] == W2[m - 1, p, 3] + X1[m, p, 3] - demand[month[m], productname[p]])
        else:
            con3[p, m] = model.addConstr(W2[m, p, 3] == X1[m, p, 3] - demand[month[m], productname[p]])
...
for p in P:
    for m in M:
        if m == 0:
            con4[p, m] = model.addConstr(W1[m, p, 3] == X1[m, p, 3])
